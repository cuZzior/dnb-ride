...
export default function EventDetailPanel({ event, onClose, onSuggestVideo }: EventDetailPanelProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
            <div
                className="glass-aurora w-full max-w-2xl max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl animate-slide-up"
            ...
            >
                ...
                <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 16rem)' }}>
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        <div className="flex items-center gap-3 text-[var(--color-text-muted)]">
                            <Clock className="w-5 h-5 text-[var(--aurora-emerald)]" />
                            <span>{formattedTime}</span>
                        </div>
                        <div className="flex items-center gap-3 text-[var(--color-text-muted)]">
                            <MapPin className="w-5 h-5 text-[var(--aurora-blue)]" />
                            <span>{event.location_name}</span>
                        </div>
                        {event.organizer && (
                            <div className="flex items-center gap-3 text-[var(--color-text-muted)]">
                                <User className="w-5 h-5 text-[var(--aurora-purple)]" />
                                <span>{event.organizer}</span>
                            </div>
                        )}
                        {event.distance !== undefined && (
                            <div className="flex items-center gap-3 text-[var(--aurora-emerald)]">
                    ...
                    <div className="flex flex-wrap gap-3">
                        <a
                            ...
                            className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-[var(--aurora-emerald)] hover:brightness-110 text-white font-medium transition-all"
                        >
                        ...
                        {!isPast && (
                            <button
                                ...
                                className="inline-flex items-center gap-2 px-5 py-3 rounded-full border border-white/20 hover:bg-white/5 text-[var(--color-text)] font-medium transition-colors"
                            >
                        ...
                        {event.event_link && (
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-2 px-5 py-3 btn-coral"
                            >
